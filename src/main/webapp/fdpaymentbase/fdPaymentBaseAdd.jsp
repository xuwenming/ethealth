...
<script type="text/javascript">
	$(function() {
	 parent.$.messager.progress('close');
		$('#form').form({
			url : '${pageContext.request.contextPath}/fdPaymentBaseController/add',
			onSubmit : function() {
		...
		<form id="form" method="post">		
			...
			<table class="table table-hover table-condensed">
				...
				<tr>	
					...
					<td>
					<input class="span2" name="expiryDate" type="text" onclick="WdatePicker({dateFmt:'<%=TfdPaymentBase.FORMAT_EXPIRY_DATE%>'})"  maxlength="0" class="" />
					</td>							
				</tr>	
				<tr>	
					<th><%=TfdPaymentBase.ALIAS_CREATE_DATE%></th>	
					<td>
					<input class="span2" name="createDate" type="text" onclick="WdatePicker({dateFmt:'<%=TfdPaymentBase.FORMAT_CREATE_DATE%>'})"  maxlength="0" class="" />
					</td>							
					<th><%=TfdPaymentBase.ALIAS_UPDATE_DATE%></th>	
					<td>
					<input class="span2" name="updateDate" type="text" onclick="WdatePicker({dateFmt:'<%=TfdPaymentBase.FORMAT_UPDATE_DATE%>'})"  maxlength="0" class="" />
					</td>							
				</tr>	
			</table>		
		</form>
