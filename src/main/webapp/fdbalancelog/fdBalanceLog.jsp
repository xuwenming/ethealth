...
<html>
<head>
<title>FdBalanceLog管理</title>
<jsp:include page="../inc.jsp"></jsp:include>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fdBalanceLogController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fdBalanceLogController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fdBalanceLogController/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/fdBalanceLogController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fdBalanceLogController/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fdBalanceLogController/view?id=' + id
		});
	}
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fdBalanceLogController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function downloadTable(){
	    ...
	    $('#downloadTable').form('submit', {
	        url:'${pageContext.request.contextPath}/fdBalanceLogController/download',
